...
- SELECT 쿼리에서 성능이 잘 나오지만, INSERT, UPDATE, DELETE 쿼리에서는 상황에 따라 다르다. 
	- UPDATE, DELETE는 WHERE 절에 잘 설정된 인덱스로 조건을 붙여주면 성능은 크게 저하되지 않고 UPDATE, DELETE를 하기 위한 데이터를 찾을 때 속도가 빨라진다. 
	- INSERT의 경우, 새로운 데이터가 추가되면서 기존 인덱스 페이지에 저장되어 있던 탐색 위치가 수정되어야 하므로 효율이 좋지 않다.


### 인덱스 설정 시 고려사항
인덱스는 하나 혹은 여러 개의 컬럼에 설정할 수 있다. DB 인덱스를 설정한다고 무조건 속도가 빨라지지는 않으므로 적절한 조건을 가지는 컬럼을 인덱스로 설정해야 한다.

1. 기수성(Cardinality)
- 기수성(Cardinality)이란
	- 특정 데이터 집합의 유니크(Unique)한 값의 개수
	- Distinct Value 개수 (= select count(distinct (column)) from table)
	- 전체 행에 대한 특정 컬럼의 데이터 중복 수치에 대한 정보
...
- 기수성이 높은 컬럼을 인덱스로 설정하였을 때 성능이 좋아진다.

2. 선택도(Selectivity)
- 선택도(Selectivity)란
	- 데이터 집합에서 특정 값을 얼마나 잘 선택할 수 있는지에 대한 지표
	- Cardinality / Total Number Of Records
- 기수성 값이 높을수록 선택도 또한 같이 높아진다. 
- 선택도가 ‘1’ 이라는 의미는 모든 값이 유일(unique)하다는 의미이다. 
...
## 이유진
### 인덱스 생성시 고려해야할 사항
1. 일반적으로 테이블 전체 row의 15% 이하의 데이터를 조회할 때 인덱스를 생성한다.
2. 데이블 건수가 적다면 굳이 테이블을 만들 필요가 없다.
3. 데이터의 유일성 정도가 좋거나 범위가 넓은 값을 가진 컬럼을 인덱스로 만드는 것이 좋다.
4. NULL이 많이 포함된 컬럼은 인덱스 컬럼으로 만들기 적당하지 않다.
5. 결합 인덱스를 만들 때에는 컬럼의 순서가 중요하다. (자주 사용되는 컬럼을 앞에 두는 것이 좋다.)
6. 테이블 당 만들 수 있는 인덱스의 수는 제한이 없지만, 너무 많이 만들면 성능 부하가 발생한다.
