...
- [Index](#index)
  ...
  - [인덱스 사용시 성능](#인덱스-사용시-성능)
  - [인덱스 설정 시 고려사항](#인덱스-설정-시-고려사항)
...
인덱스는 하나 혹은 여러 개의 칼럼에 대해 설정할 수 있다. 단일 인덱스를 여러 개 생성할 수도 있고, 여러 칼럼을 묶어 복합 인덱스를 설정할 수도 있다. 인덱스를 사용할 경우 보통 검색(`select`) 속도가 빨라진다.  

#### 인덱스가 DML(Data Manipulation Language) 미치는 영향

> 인덱스가 쿼리의 성능을 향상시키니까 인덱스를 많이 적용하면 어떻게 될까?
> 
- 삽입
    - 인덱스에 대한 데이터도 추가해야 되고, 인덱스를 정렬해야 하기 때문에 성능손실이 생긴다.
- 삭제
    - 인덱스에 존재하는 값은 삭제하지 않고 사용안한다는 표시로 남게되므로 공간 낭비를 일으킨다.
    (데이터를 찾을 때는 빠르다.)
- 갱신
    - 삽입,삭제의 문제점이 동시에 생김 (원래 데이터를 삭제후 삽입하는것이기때문)
    (데이터를 찾을 때는 빠르다.)

즉, 인덱스를 무조건 많이 설정하는게 속도 향상을 높이진 않는다. 오히려 성능과 공간의 손실이 생길 수 있다. 

또한, 데이터의 삽입, 삭제, 수정이 빈번히 일어나는 테이블보단 **조회를 자주하는 테이블에서 주로 사용하는 컬럼의 인덱스를 생성하는 것이 유리**하다. 인덱스는 데이터베이스 메모리를 사용하여 테이블 형태로 저장되므로 개수와 저장 공간은 비례 한다. 따라서 **조회 시 자주 사용**하고 **고유한 값 위주**로 인덱스를 설정하는 것이 좋다.
...
### 인덱스 설정 시 고려사항
 ...
 DB 인덱스를 설정한다고 무조건 속도가 빨라지지는 않으므로 적절한 조건을 가지는 컬럼을 인덱스로 설정해야 한다.

- 새로 추가되는 인덱스는 기존 액세스 경로에 영향을 미칠 수 있음
- 인덱스는 하나 혹은 여러 개의 컬럼에 설정할 수 있다. 지나치게 많이 만들면 오버헤드가 발생
- 넓은 범위를 인덱스로 처리하면 많은 오버헤드가 발생
- 인덱스를 만들면 추가적인 저장 공간이 필요
- 인덱스와 테이블 데이터의 저장 공간이 분리되도록 설계
- 인덱스들이 자주 조합되어 사용되는 경우 하나의 결합 인덱스로 생성
    - 결합 인덱스를 만들 때에는 컬럼의 순서가 중요함
    - 자주 사용되는 컬럼을 앞에 두는 게 좋음
- 데이터 형식에 따라 인덱스의 효율성이 달라짐
- 데이블 건수가 적다면 굳이 테이블을 만들 필요가 없다.
- 일반적으로 테이블 전체 row의 15% 이하의 데이터를 조회할 때 인덱스를 생성한다.
- 컬럼 조건
    - 입출력 장표 등에서 조회 및 출력 조건으로 사용되는 컬럼
    - 인덱스가 자동 생성되는 기본키와 Unique키 제약 조건을 사용한 컬럼
    - 가능한 한 수정이 빈번하지 않은 컬럼
    - NULL이 많이 포함되지 않은 컬럼
    - 데이터의 유일성 정도가 좋거나 범위가 넓은 값을 가진 컬럼

#### 선택도 (Selectivity)

- 선택도
    - 전체 레코드 중 **조건절에 의해 선택될 것으로 예상되는 레코드 비율(%)**
    - 데이터에서 특정 값을 얼마나 잘 선택할 수 있는지에 대한 지표이다.
    - 선택도가 ‘100%’ 이라는 의미는 모든 값이 같다는 의미이다.
- 선택도 산정 방법 (히스토그램이 없을 때 기준)
    - 등치 조건(`=`)에 대한 선택도
    `선택도 = 1 / NDV * 100`  (NDV == Number of Distinct Value)
    - 범위 조건(부등호, between)에 대한 선택도
    `선택도 = 특정 값의 row 개수 (or 값들의 평균 row 개수) / 전체 row 개수 * 100` 
    `선택도 = 칼럼 값들의 평균 row 개수 / 전체 row 개수 * 100`
- 높은 선택도를 가지는 컬럼을 인덱스로 설정하는 경우 조회 성능이 향상된다.
선택도가 높을수록 검색 대상이 줄어들기 때문이다.
선택도가 좋지 않아도, 정렬이나 그룹핑같은 작업을 위해 인덱스를 만드는 게 나은 경우가 있다.
- 5~10% 정도가 적당하다.
5% ~ 10% 이상이어도 부분 처리를 목적으로 하는 컬럼

#### 기수성 (Cardinality)

- 기수성
    - Distinct Value 개수 (= `select count(distinct (column)) from table`)   
    즉, **출력될 것으로 예상되는 결과 건수**
...
- 카디널리티 구하기 (히스토그램이 없는 경우)
    - `카디널리티 = 총 row 수 * 선택도`
- 중복되는 횟수가 높으면 기수성 값이 낮고, 중복되는 횟수가 낮으면 기수성 값이 높다고 표현한다.
- 데이터의 중복되는 횟수가 낮은 컬럼을 이용하여 데이터를 그룹 지으면 그룹에 편성되는 데이터의 수가 적어지므로 탐색이 빨라지는 효과를 얻게 된다.
- 카디널리티가 높을수록 인덱스로 설정했을 때 성능이 좋아지므로, 인덱스 설정에 좋은 칼럼이다.
...
- 활용도란
    ...
    - 수동 쿼리 조회, 로직과 서비스에서 쿼리를 날릴 때 **WHERE 절에 자주 활용되는지**를 판단하면 된다.
