...
## 이유진
### JVM (Java Virtual Machine)
![](https://t1.daumcdn.net/cfile/tistory/99960C505C37390616)
- 자바 어플리케이션을 클래스 로더를 통해 읽어서 자바api와 함께 실행시키는 스택 기반의 가상 머신이다. 
- Java와 운영체제 사이의 중계자 역할을 하여 OS에 독립적으로 자바를 사용할 수 있게 해준다.
- 자바 프로그램을 실행시킨다 == JVM을 실행시키고 그 위에서 자바 프로그램을 실행시킨다
- 메모리 관리 및 GC를 수행한다.

### JAVA프로그램 실행 과정
1. 프로그램이 실행되면 JVM은 OS로부터 프로그램이 필요로 하는 메모리를 할당받는다.
2. 자바 컴파일러(javac)가 자바 소스코드(.java)를 읽어서 자바 바이트 코드(.class)로 컴파일한다.
3. 클래스 로더는 동적 로딩을 통해 필요한 클래스들을 로딩 및 링크하여 JVM의 메모리(런타임 데이터 영역 중 메소드 영역)에 올린다.
4. Execution engine은 JVM메모리에 올라온 바이트 코드들을 명령어 단위로 하나씩 가져와서 실행한다. 

#### 자바 바이트 코드
JVM이 이해할 수 있는 코드. 컴퓨터는 이해할 수 업는 반기계어.

#### 클래스 로더
JVM내로 클래스 파일을 로드하고, 링크를 통해 배치하는 작업을 수행하는 모듈이다. 런타임 시에 동적으로 클래스를 로드한다.

#### 실행 엔진의 동작방식 (2가지)
> 실행엔진 : 바이트코드로 된 `.class` 파일을 실행한다.

- 자바 인터프리터 : 바이트 코드 명령어를 한줄씩 읽어서 해석하고 실행한다. 각각의 실행은 빠르지만, 전체적인 실행 속도가 느리다.
- JIT컴파일러(just-in-time compiler) : 인터프리터의 단점을 보완하기 위해 도입된 방식이다. 바이트 코드 전체를 바이너리코드로 컴파일하고, 바이너리코드로 직접 실행하는 방식이다. 전체적인 실행속도는 인터브리팅 방식보다 빠르다. 단, 컴파일 시 비용이 발생한다.
=> JVM에서는 인터프리터 방식을 사용하다 일정 기준이 넘어가면 JIT컴파일러 방식으로 실행된다.
...
JVM의 메모리 영역으로 자바 앱 실행 시 사용되는 데이터들을 적재하는 영역이다.
![image](https://user-images.githubusercontent.com/37680108/137890551-0f58b4f3-4f9e-43ef-9418-5f23baa5d7ea.png)
#### Method Arda
- 모든 쓰레드가 공유하는 메모리 영역
- 클래스, 인터페이스, 메소드, 필드, static변수 등의 바이트 코드를 보관

#### Heap
- 모든 쓰레드가 공유하는 메모리 영역
- new 키워드로 생성된 객체와 배열이 저장되는 영역
- method영역에 로드된 클래스만 생성 가능하다
- GC가 수행되는 영역

#### Java Stack
- 메서드 호출 시마다 각각의 스택 프레임이 생성되고, 메서드 수행이 끝나면 프레임별로 삭제한다.
- 각각의 스택 프레임에 메서드에서 사용되는 값들을 저장하고, 호출된 메서드의 매개변수, 지역변수, 리턴 값 등을 임시로 저장한다.

#### PC register
- 쓰레드가 시작될 때 생성되고, 쓰레드마다 하나씩 존재한다.
- 쓰레드가 어디서 무엇을 실행해야 하는지 기록하는 부분으로, 현재 수행중인 JVM명령주소를 갖는다.
- CPU의 Register와 역할이 비슷하다.

#### Native Method Stacks
- 자바 이외의 언어로 된 코드를 위한 스택이다.
- JNI(Java Native Interface)를 통해 호출되는 다른 언어의 코드(메서드 호출 등)를 수행하기 위해 존재한다.
